...
class PokerGame:
    ...
    def _is_betting_round_complete(self) -> bool:
        ...
        # 所有可行动的玩家都已行动过，且下注金额相同
        all_acted = all(p.has_acted for p in active_can_act)
        all_even = all(p.current_bet == self.current_bet for p in active_can_act)
        return all_acted and all_even
